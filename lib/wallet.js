...
wallet.fromV3 = function (v3, password, callback) {
  try {
    ...
    const cb = function (derivedKey) {
      ...
      const decipher = crypto.createDecipheriv(v3.crypto.cipher, derivedKey.slice(0, 16), new Buffer(v3.crypto.cipherparams.iv, 'hex'));
    ...
    if (v3.crypto.kdf === 'scrypt') {
      ...
      crypto.pbkdf2(new Buffer(password), new Buffer(kdfparams.salt, 'hex'), kdfparams.c, kdfparams.dklen, 'sha256', (err, derivedKey) => {
        if (err) {
          callback(err, null);
          return;
        }
