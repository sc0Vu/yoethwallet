...
wallet.generate = function (mnemonic, hdPath, callback) {
  try {
    ...
    const hdKey = HDKey.fromMasterSeed(Buffer.from(randomSeed, 'hex'));
...
wallet.fromV3 = function (v3, password, callback) {
  try {
    ...
    const encCB = function (err, derivedKey) {
      ...
      const decipher = crypto.createDecipheriv(v3.crypto.cipher, derivedKey.slice(0, 16), Buffer.from(v3.crypto.cipherparams.iv, 'hex'));
    ...
    if (v3.crypto.kdf === 'pbkdf2') {
      ...
      crypto.pbkdf2(Buffer.from(password), Buffer.from(kdfparams.salt, 'hex'), kdfparams.c, kdfparams.dklen, 'sha256', (err, derivedKey) => {
        if (err) {
          callback(err, null);
          return;
        }
      ...
      scrypt(password, saltBuffer, kdfparams.dklen, {
        N: kdfparams.n, r: kdfparams.r, p: kdfparams.p, maxmem,
      }, encCB);
    } else {
